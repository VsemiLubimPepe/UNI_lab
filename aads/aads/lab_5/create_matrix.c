...
int main(int argc, char** argv)
{
    ...
    FILE *matrix_a_file = fopen("matrix_a", "w");
    if (!matrix_a_file) {
	printf("Couldn't open file to write matrix values.\n");
	return -1;
    }

    fprintf(matrix_a_file, "%d %d \n", order, order);
	...
	for (size_t j = 0; j < order; j++) {
	    fprintf(matrix_a_file, "%f %f*i ",
		    get_random_double(-10.0, 10.0), get_random_double(-10.0, 10.0));
	}
    ...
    FILE *matrix_b_file = fopen("matrix_b", "w");
    if (!matrix_b_file) {
	printf("Couldn't open file to write matrix values.\n");
	return -1;
    }

    fprintf(matrix_b_file, "%d %d \n", order, order);
